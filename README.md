<h1><img src="https://emojis.slackmojis.com/emojis/images/1643514738/7421/typingcat.gif?1643514738" width="30"/> Hey! Welcome to my GitHub</h1>

<p>I'm <b>Rodaina Mohammed ElHady</b>, a passionate student and aspiring IT professional from <img src="https://cdn-icons-png.flaticon.com/512/14009/14009645.png" width="13"/> <b>Giza, Egypt</b>. I explore web development, penetration testing, AI, and IT support projects.</p>

<h3>📫 Contact</h3>
<p>
  Email: <b>rodaina.mohammed@outlook.com</b><br/>
  LinkedIn: <a href="https://www.linkedin.com/in/rodaina-othman">rodaina-othman</a><br/>
  GitHub: <a href="https://github.com/rodainaelhady">rodainaelhady</a>
</p>

<h3>🎓 Education</h3>
<p>
  Fourth-year student, Faculty of Technology, Helwan International Technology University<br/>
  Major: Cybersecurity | Grade: Excellent
</p>

<h3>💼 Internships / Training</h3>
<ul>
  <li><b>Penetration Testing Internship</b> - Cyber X | Remote, Egypt (Aug 2024)  
  Remote penetration testing using Burp Suite, Nmap, and Wireshark under expert supervision.</li>
  <li><b>CCNA / Cybersecurity Program</b> - Ministry of Military Production | On-site, Egypt (Jul 2024)  
  Network building with Cisco technologies; studied common cyber-attacks.</li>
</ul>

<h3>🛠 Skills</h3>
<p>
<b>Programming & Scripting:</b> Python, PHP, C++, Java, JavaScript, Bash, SQL, HTML, CSS<br/>
<b>Networking & Security Tools:</b> Wireshark, Nessus, Burp Suite, Nmap<br/>
<b>Soft Skills:</b> Time Management, Communication, Teamwork<br/>
<b>Languages:</b> Arabic (Native), English (Intermediate – B1+)
</p>

<h3>💻 Projects</h3>
<table>
  <thead align="center">
    <tr>
      <td><b>Project</b></td>
      <td><b>Description</b></td>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><a href="https://github.com/rodainaelhady/AI-multimedia-tools"><b>AI Multimedia Tools</b></a></td>
      <td>Python + CTK automation for system maintenance: startup management, speed tests, cleanup.</td>
    </tr>
    <tr>
      <td><a href="https://github.com/rodainaelhady/Attendance-System"><b>Attendance System</b></a></td>
      <td>QR-based check-in system with face/signature recognition; auto-generates QR codes from Excel.</td>
    </tr>
    <tr>
      <td><a href="https://github.com/rodainaelhady/Info-Gathering-Tool"><b>Info Gathering Tool</b></a></td>
      <td>Automates WHOIS, subdomains, paths, and email collection using Python + CTK.</td>
    </tr>
  </tbody>
</table>

<h3>📚 Courses</h3>
<ul>
  <li>Vulnerability Assessment & Penetration Testing – DEPI (2025)</li>
  <li>Google Technical Support Certificate – DEPI (2024)</li>
  <li>Web Design Course – NTI (2024)</li>
  <li>Cybersecurity Course – ITI (2023)</li>
</ul>

<h3>⚡ Tools & Technologies</h3>
<p>
  <img alt="Python" src="https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img alt="PHP" src="https://img.shields.io/badge/-PHP-777BB4?style=flat-square&logo=php&logoColor=white" />
  <img alt="JavaScript" src="https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
  <img alt="HTML5" src="https://img.shields.io/badge/-HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" />
  <img alt="CSS3" src="https://img.shields.io/badge/-CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" />
  <img alt="MongoDB" src="https://img.shields.io/badge/-MongoDB-13aa52?style=flat-square&logo=mongodb&logoColor=white" />
  <img alt="Kali Linux" src="https://img.shields.io/badge/-Kali_Linux-557C94?style=flat-square&logo=Kali-Linux&logoColor=white" />
</p>

<p align="center">This <i>README</i> is manually maintained and updated regularly!</p>
